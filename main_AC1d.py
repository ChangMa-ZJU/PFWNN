...
class Example():
    ...
    def get_error(self, loadfig_path, savefig_path, data_path, net_type:str, names:list, num:int=1):
        ...
        for i in range(len(names)):
            ...
            for j in range(num):
                ...
                #
                abs_error_best[i,j] = np.max(np.abs(data_test['u_pred_preall'][:,0] - data_test['u_test_preall'][:,0]))
        ...
        #
        self.t, self.x, self.u = data_test['t_test_preall'], data_test['x_test_preall'], data_test['u_test_preall'][:,0]
        #
        ...
        # Maximum Bound Priciple
        Draw().MBP([self.t] , [self.x] , [u_pred_list],
                                  label_list=[ r'Predicted $\phi_{{}_{NN}}$'],
                                  save_path=savefig_path + "AC1d_MBP.pdf")
        # log-log
    ...
    def solve(self, step, stage,solver):
        ...
        max_boundary, max_initial, max_interior = solver.MBP()

        return max_boundary, max_initial, max_interior

    def WNN(self, t1, t2, step, stage, inx:int, name:str='PFWNN'):
        #
        from Solvers.PFWNN_AC import PFWNN
        #
        kwargs = {'Num_particles': 50, 'Num_tin_size': 60, 'Nx_integral': 5, 'Nt_integral': None, 'topk': 4000,
                  'train_xbd_size_each_face': 1, 'train_tbd_size': 200, 'train_init_size': 512, 'maxIter': 50000,
                  'lr': 1e-3, 'w_weak': 100., 'tol': 0.1, 'w_bd': 5., 'w_init': 100., 'R_way': 'Rdescend',
                  'R_max': 1e-4, 'R_min': 1e-6, 'net_type': 'tanh_per', 'test_fun': 'Wendland', 'int_method': 'mesh',
                  'hidden_width': 128, 'hidden_layer': 4, 'dtype': {'numpy': self.np_type, 'torch': self.torch_type},
                  't1': t1, 't2': t2, 'stage': stage, 'step': step, 'interval': time_interval, 'loadpath': data_path,
                  'coe_M': 5, 'coe_lam2M': 0.0001, 'Nc_append': 50, 'ada_step': 10, 'num_L': 5,
                  'saved_path': current_directory + '/SavedModels/PFWNN/' + model_name + f"/4/"}
        #
        print("The parameters are:", kwargs)
        #
        solver = PFWNN(Problem=Problem, **kwargs)
        max_boundary, max_initial, max_interior = self.solve(step = kwargs['step'], stage = kwargs['stage'],solver=solver)
        np.save(kwargs['saved_path'] + 'settings.npy', kwargs)
        print('max_bdry, max_init, max_initeror:', max_boundary,max_initial,max_interior)

if __name__=='__main__':
    from Problems.AllenCahn1d import Problem
    #
    demo = Example(np_type=np.float32, torch_type=torch.float32)
    time_group = [[0., 1., 0, 200]]
    time_interval = 200
    model_name = "AC_1d_1e-5"
    loadfig_path = current_directory + '/SavedModels/PFWNN/' + model_name + '/4/'
