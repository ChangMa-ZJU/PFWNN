...
class TestFun():
    ...
    def _Bump(self, x_scaled:torch.tensor, dim:int)->torch.tensor:
        '''
        Input:
            x_scaled: (?, d) or (?, m, d)
        '''
    ...
    def _Wendland(self, x_scaled:torch.tensor, dim:int)->torch.tensor:
        '''
        Input:
            x_scaled: (?, d) or (?, m, d)
        '''
    ...
    def _Wendland_1(self, x_scaled: torch.tensor, dim: int) -> torch.tensor:
        '''
        Input:
            x_scaled: (?, d) or (?, m, d)
        '''
        #
        l = math.floor(dim / 2) + 2
        #
        r = 1. - torch.relu(1. - self._dist(x_scaled))
        r_list = [r]
        for _ in range(1):
        ...
        v = (1 - r) ** (l + 1) * ((l + 1.) * r + 1.)
        #
        dv_dr_divide_by_r = -r * (1 - r) ** l*  (l ** 2 + 3. * l + 2.)
        ddv_dr = (1 - r) ** l * ((l ** 4 + 11. * l ** 3 + 43. * l ** 2 + 69. * l + 36) * r_list[1] - (
                    l ** 3 + 7. * l ** 2 + 12 * l) * r - (l ** 2 + 7. * l + 12)) / 3.
        if dim == 1:
            dv = dv_dr_divide_by_r * r * torch.sign(x_scaled)
            Lv = ddv_dr
        else:
    ...
    def _Wendland_3(self, x_scaled: torch.tensor, dim: int) -> torch.tensor:
        '''
        Input:
            x_scaled: (?, d) or (?, m, d)
        '''
        #
        l = math.floor(dim / 2) + 4
        #
        r = 1. - torch.relu(1. - self._dist(x_scaled))
        r_list = [r]
        for _ in range(1):
            r_list.append(r * r_list[-1])
        #
        v = ((1 - r)**(l + 3)*((l**3 + 9*l**2 + 23*l + 15)*r**3 + (6*l**2 + 36*l + 45)* r**2 + (15*l + 45)*r + 15))/15
        #
        dv_dr_divide_by_r =-(r * (r - 1) ** 2 * (1 - r) ** l * (l ** 2 + 11 * l + 30) *
                             (l ** 2 * r ** 2 + 4 * l * r ** 2 + 3 * l * r + 3 * r ** 2 + 6 * r + 3)) / 15
        ddv_dr = (1 - r) ** l * ((l ** 4 + 11. * l ** 3 + 43. * l ** 2 + 69. * l + 36) * r_list[1] - (
                    l ** 3 + 7. * l ** 2 + 12 * l) * r - (l ** 2 + 7. * l + 12)) / 3.
        if dim == 1:
            dv = dv_dr_divide_by_r * r * torch.sign(x_scaled)
    ...
    def get_value(self, x_scaled:torch.tensor)->torch.tensor:
        ...
        elif self._type=='Wendland_1':
            return self._Wendland_1(x_scaled=x_scaled, dim=self._dim)
        elif self._type=='Wendland_3':
            return self._Wendland_3(x_scaled=x_scaled, dim=self._dim)
        else:
