...
class Example():
    ...
    def get_error(self, loadfig_path, savefig_path,data_path, net_type:str, names:list, num:int=1):
        ...
        #                    confid=True,
        #                    save_path=savefig_path+'CHip1d_l2err_time.pdf')
        # ############### loss vs. time(s)
        # Draw().show_confid_time(times_list, loss_list,
        #                    x_name=r'Time(s)', y_name=r'Loss',
        #                    confid=True,
        #                    save_path=savefig_path+'CHips1d_loss_time.pdf')
        ...
        #                    confid=True,
        #                    save_path=savefig_path+"CH1dip_E_loss.pdf")
        ############### energy function
        Draw().show_energyfun([self.u, self.u_pred], [self.E_test, self.E_pred],
                                ...
                                save_path=savefig_path+"CH1dip_contrast_0.pdf")
        ############## point-wise error
        ...
        #                                       r'|$\phi^{NN} -\phi$|'],
        #                           save_path= savefig_path+ 'CHfp1d_pointwise_0.pdf')
        ...
        #                                       r'|$f^{NN}(\phi^{NN}) -f(\phi)$|'],
        #                           save_path=savefig_path+ 'CHip1d_energy_pointwise.pdf')
        # # log-log
        # Draw().loglog(loss_list, E_errs_list, xlabel=r'Loss', ylabel=r'Relative error of $f$',
        #               save_path=savefig_path + "log_log_f.pdf")
        # ############### timestamp error
        ...
        #
        symbol_express = Draw().symbol_expression(self.u_pred,self.E_pred)
        print('The symbol of expression is', symbol_express)
    ...
    def WNN(self, port_x, port_t, inx:int, name:str='PFWNN'):
        '''
        '''
        from Solvers.PFWNN_CH_inverseF import PFWNN
        ############ inverse problem
        kwargs = {'Num_particles': 150,
                  'Num_tin_size': 100,
                  'Nx_integral': 10,
                  'Nt_integral': None,
                  'topk': 1500,
...
if __name__=='__main__':
    ...
    errall = []
    # for port in portion_group:
    #     print('The portion is:', port)
    model_name = "CHIP_1d"
    t1 = 0.
    t2 = 1.
    ...
    data_path = data_path = current_directory + "/Data/cahn-hillard1d.mat"
    #
    # demo.WNN(port_x=2, port_t=2,inx=0)
    # #########
    # if not os.path.exists(savefig_path):
        # os.makedirs(savefig_path)
    # demo.get_error(loadfig_path=loadfig_path, savefig_path=savefig_path, data_path =data_path, net_type='tanh_per', names=['PFWNN'], num=1)
