#  * @Author: chang.ma
'''
according to the weak residual
'''
import numpy as np
import scipy.io
import time
...
class PFWNN(Module.Solver):
    ...
    def __init__(self, Problem: Problem,
        ...
        self.xbd = train_xbd_size_each_face
        self.tbd = train_tbd_size
    ...
    def _save(self, model_type: str) -> None:
        ...
        if model_type == 'model_final':
            ...
            # dict_loss['error_preall'] = self.error_preall
    ...
    def test(self, step: int, stage: int, model_type='model_best_loss') -> None:
        ...
        if self.stage == 0:
            ...
            u_pred_preall = scipy.io.loadmat(parent_directory_path + '/' + str(self.stage - 1) + f'/test_saved.mat')[
                'u_pred_preall']
    ...
    # Maximum Bound Prinple
    def MBP(self, model_type='model_best_loss') -> None:
        '''
        '''
        stage = self.stage
        step = self.step
        ...
        x_lb, x_ub, t_bd = self.data.get_bd(t0=self.t1, tT=self.t2, Nt_size=self.tbd, Nx_bd_each_face=self.xbd)
        # u_init_true, x_init, t_init = self.problem.fun_u_init()
        x_init, t_init = self.data.get_init(Nx_init= self.train_init_size)
        # u_test, x_test, t_test, u0_test, x0_test, t0_test, upre_valid, xpre_valid, tpre_valid = \
        #     self.problem._valid_u_timestep(self.t1, self.t2, self.interval)
        x_in, t_in = self.data.get_in(t0=self.t1, tT=self.t2, Nt_size=100, Nx_size=200)
        with torch.no_grad():
            # bound value
            u_lb_nn = self.model_u(torch.cat([t_bd, x_lb], dim=1))
            u_ub_nn = self.model_u(torch.cat([t_bd, x_ub], dim=1))
            # initial value
            u_init_pred = self.model_u(torch.cat([t_init, x_init], dim=1))
            # initilal value
            # _, x_init, t_init = self.problem.fun_u_init()
            # u_init_pred = self.model_u(torch.cat([t_init, x_init], dim=1))
            # initerior point
            u_pred_initeior = self.model_u(torch.cat([t_in, x_in], dim=1))

        max_boundary = torch.abs(torch.max(torch.max(u_lb_nn), torch.max(u_ub_nn)))
        max_initial = torch.abs(torch.max(u_init_pred))
        max_interior = torch.abs(torch.max(u_pred_initeior))

        return max_boundary, max_initial, max_interior
    ...
    def get_net(self) -> None:
        ...
        # self.model_u = torch.nn.DataParallel(model_u, device_ids=device_ids)
        #
    ...
    def get_loss(self, x_scaled, xc, tc, R, w_weak, w_init, **args):
        ...
        weak_form_0 = (weak ** 2).view(self.Num_tin_size, self.Num_particles)
        # self-adaptive weight
        sums = weak_form_0.sum(dim=1, keepdim=True).detach()
        weight = weak_form_0.detach() / sums * self.Num_particles
        weak_form = weak_form_0 * weight
        #
        L_t = torch.mean(weak_form, dim=1)
        W = torch.exp(-self.tol * (self.M @ L_t)).data.to(device)
        loss_equ = torch.mean(W * L_t) * w_weak  # self.w_weak
        ...
        return loss, loss_equ, loss_init, W, weak_form_0, xc, tc
    ...
    def train(self) -> None:
        ...
        for iter in range(self.iters):
            ...
            loss_u_train, loss_equ_train, loss_init_train, W, weak_residual, xc, tc = (
            ...
            with (torch.no_grad()):
                ...
                if iter % self.ada_step == 0:
                    if self.problem.dim == 1:
                        ...
                        top_tc = top_tc_0.view(-1,1,1)
                        #
                        _, xc, tc = self.data.get_txc(t1=self.t1, t2=self.t2, N_xc=(self.Num_particles-self.Nc_append),
                                                      Nt_size=self.Num_tin_size, R_max=self.Rmax, R_min=self.Rmin)
                        xc = torch.cat((xc, top_xc), dim=0)
                        tc = torch.cat((tc, top_tc), dim=0)
                        #
                ...
                #
                if iter % 100 == 0:
                    print(f"Iter:{iter + 1},error:{self.error[-1]:.4f},loss_u:{np.mean(self.loss_u_list[-5:]):.8f},"
                          f"loss_equ:{np.mean(self.loss_equ_list[-5:]):.8f},loss_init:{np.mean(self.loss_init_list[-5:]):.8f},"
                          f"weigh-1:{W[-1]},time:{self.time_list[-1]:.4f}")
