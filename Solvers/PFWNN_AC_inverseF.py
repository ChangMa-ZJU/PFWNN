...
class PFWNN(Module.Solver):
    ...
    def __init__(self, Problem:Problem, 
                 ...
                 lr:float, net_type_u:str,net_type_E:str, **kwargs):
        #
        ...
        #
        self.problem = Problem( loadpath=kwargs['data_path'], M=kwargs['coe_M'], lambda2_M=kwargs['coe_lam2M'], test_type=kwargs['test_fun'], dtype=self.dtype['numpy'])
    ...
    def get_loss(self, **args):
        ...
        #
        weak = self.problem.weak_inverseF(self.model_u, self.model_E, x_scaled.to(device), xc.to(device), tc.to(device), R.to(device), device=device) 
        weak_form = (weak ** 2).view(self.Num_tin_size, self.Num_particles)
        L_t = torch.mean(weak_form, dim=1)
        W = torch.exp(-self.tol * (self.M @ L_t)).data.to(device)
        loss_equ = torch.mean( W * L_t )* self.w_weak
        ...
        loss_init = torch.mean( (u_init_pred - u_init_true) **2 ) * self.w_init
        ########## mismatch inside the domain (data loss in inverse problem)
        # u_valid, x_valid, t_valid = self.problem._fun_u_portion(self.portion_x, self.portion_t)
        u_valid, x_valid, t_valid, _, _, _, _, _, _ = \
            self.problem._valid_u_timestep(t1=self.t1, t2=self.t2, interval=self.interval)
        u_pred_in = self.model_u(torch.cat([t_valid.to(device), x_valid.to(device)], dim=1))
        #
        u_mea_in = u_valid.to(device) + torch.randn_like(u_pred_in) * self.noise_level* torch.std(u_valid.to(device), unbiased=False)
        loss_data = torch.mean((u_pred_in - u_mea_in)**2 )*self.w_data
        loss = loss_equ + loss_init + loss_data

        return loss, loss_equ, loss_init, loss_data, W
